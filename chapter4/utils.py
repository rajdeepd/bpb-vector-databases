...
def construct_HNSW_2(vec_pos, m_nearest_neighbor):
    max_layers = 4
    vec_num = vec_pos.shape[0]

    # Precompute distance matrix
    dist_mat = np.linalg.norm(vec_pos[:, np.newaxis] - vec_pos, axis=2)

    # Assign random layer to each node
    node_layer = [layer_num(max_layers) for _ in range(vec_num)]
    max_layer = max(node_layer) + 1  # layer indices start from 0
    ...
    for layer in range(max_layer):
        ...
        # Nodes active in this layer
        active_nodes = [i for i in range(vec_num) if node_layer[i] >= layer]
        for i in active_nodes:
            G.add_node(i, pos=vec_pos[i])

        # Add edges
        for i, node_i in enumerate(active_nodes):
            candidates = active_nodes[:i]
            if not candidates:
                continue

            dists = dist_mat[node_i, candidates]
            k = min(m_nearest_neighbor, len(candidates))
            nearest_indices = np.argsort(dists)[:k]
            for idx in nearest_indices:
                node_j = candidates[idx]
